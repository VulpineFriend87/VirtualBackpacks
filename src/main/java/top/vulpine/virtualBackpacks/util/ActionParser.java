...
public class ActionParser {
    ...
    public ActionParser(VirtualBackpacks plugin) {
        this.plugin = plugin;
    }
    ...
    public void executeActions(List<String> actions, Player player, int currentIndex, Map<String, String> placeholders) {
        ...
        for (int i = currentIndex; i < actions.size(); i++) {
            ...
            try {
                ...
                String raw = actionString.trim();
                int openBracketIndex = raw.indexOf('[');
                int closingBracketIndex = (openBracketIndex >= 0) ? raw.indexOf(']', openBracketIndex + 1) : -1;
                if (openBracketIndex != 0 || closingBracketIndex == -1) {
                    Logger.warn("Invalid action format: " + actionString);
                    continue;
                }

                String action = raw.substring(openBracketIndex + 1, closingBracketIndex).trim();
                String params = raw.substring(closingBracketIndex + 1).trim();
    ...
    private void executeCommand(String params, Player player) {
        ...
        if (parts.length < 2) {
            Logger.warn("Invalid COMMAND params: " + params);
            return;
        }
